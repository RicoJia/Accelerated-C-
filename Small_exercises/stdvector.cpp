...
//------------------------------------------------------------------------------------------------
int main(){
vector<int> a = {1,2,3,4,5,6};     // this is how you initialize a vector. you don't need size. 
print1vec(a);
...
cout<<"the capacity of a now is: "<<a.capacity()<<endl;     // even though you cleared it, the capacity is the length before clearing. 
cout<<"the max size of a now is: "<<a.max_size()<<endl;}	//this tells u the maximum size, which is 452341234... elements.
...
auto j = a.begin()+4;			// insert the element as the 5th.  element.  resize is useless. 
a.insert(j,4); 
a.pop_back();
print1vec(a);
}
...
for (auto i = vec.begin(); i!=vec.end();i++)	   // this i show you use iterator. it is similar to a pointer. ALTERNATIVELY: you can use for each loop as well. 
	{
	cout<<*i<<" ";				   //you need to dereference the iterator!!   pay attention to the sequence of * and i!
	}
	cout<<endl;
cout<<"the front is: \n";
cout<<vec.front()<<endl<<"the back is:\n";	  // this is how u use front() and back(). u get the element directly. 
cout<<vec.back()<<endl<<endl; 

auto j = vec.begin()+vec.size()/2;		// you can't add two iterators together: auto j = (vec.begin()+vec.end()+1)/2; you can add a number to it tho. it's like adding two pointers together. 
